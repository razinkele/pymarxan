...
**Space targets.** A feature carries an optional `space_target ∈ [0, 1]` (a proportion; absent /
0 = no space target — behaviour unchanged). Configured via a features-table column or a
`SpaceTargetSpec`.
